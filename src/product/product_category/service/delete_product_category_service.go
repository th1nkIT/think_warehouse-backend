...
func (s *ProductCategoryService) DeleteProductCategory(ctx context.Context, guid string, userData sqlc.GetUserBackofficeRow) (err error) {
	...
	if err = q.DeleteProductCategory(ctx, sqlc.DeleteProductCategoryParams{
		...
		log.FromCtx(ctx).Error(err, "failed delete product category")
		err = errors.WithStack(httpservice.ErrUnknownSource)
