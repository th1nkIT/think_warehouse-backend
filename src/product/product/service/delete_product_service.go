...
func (s *ProductService) DeleteProduct(ctx context.Context, guid string, userData sqlc.GetUserBackofficeRow) (err error) {
	...
	if err = q.DeleteProduct(ctx, sqlc.DeleteProductParams{
		...
		log.FromCtx(ctx).Error(err, "failed delete product")
		err = errors.WithStack(httpservice.ErrUnknownSource)
